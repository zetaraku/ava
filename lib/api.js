...
const debugGlob = createDebug('ava:glob');
...
export default class Api extends Emittery {
	...
	async run({files: selectedFiles = [], filter = [], runtimeOptions = {}} = {}) { // eslint-disable-line complexity
		...
		let testFiles;
		try {
			debugGlob('Find tests');
			testFiles = await globs.findTests({cwd: this.options.projectDir, ...apiOptions.globs});
			debugGlob('Found %d test files, now filtering', testFiles.length);
			...
			debugGlob('Selected %d test files', selectedFiles.length);
		} catch (error) {
			selectedFiles = [];
