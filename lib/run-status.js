...
const debugEvent = ({type, testFile, ...event}) => {
	debugEvent[type] ??= createDebug(`ava:event:${type}`)
	if (testFile) {
		debugEvent[type]('%s %O', path.relative(process.cwd(), testFile), event)
	} else {
		debugEvent[type]('%O', {testFile, ...event})
	}
}
